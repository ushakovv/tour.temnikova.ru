...
class Tabular extends Widget
{
    ...
    public function run()
    {
        ...
        $relQuery = $this->model->$relQueryGetter();
        ...
        foreach ($items as $index => $item) {
            $output .= self::_renderTabularItem($this->form, $this->template, $index, $item);
        }
        ...
        $output = '<div class="tabular_block">'
            ...
            . self::_renderTabularItem($this->form, $this->template, '{index}', new $relModelClass())
            . '</div></div>';


        UpdateScenario::registerHandler(
            null,
            '\core\widgets\Tabular',
            'update',
            [
                'relation' => $relation
            ],
